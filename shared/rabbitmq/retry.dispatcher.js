...
export const startRetryDispatcher = async (
  ...
  for (const delay of RETRY_CONFIG.delays) {
    ...
    const consumer = await channel.consume(
      ...
      async (message) => {
        ...
        try {
          ...
          console.log(
            `[RabbitMQ] Retry dispatched: ${originalRoutingKey}`
          );
          ...
          /*
           * Keep retry message in the
           * retry queue if publishing fails.
           */
    ...
    console.log(
      `[RabbitMQ] Retry dispatcher listening on ${retryQueue}`
    );
  }
