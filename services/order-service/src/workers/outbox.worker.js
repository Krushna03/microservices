...
const WORKER_ID = `order-outbox-${crypto.randomUUID()}`;
...
export const processOutbox = async () => {
  ...
  console.log(`[Order Outbox] Worker ${WORKER_ID} claimed ${events.length} events`);

  for (const event of events) {
    try {
      console.log(`[Order Outbox] Publishing event ${event.eventId} (${event.eventType})`);
      ...
      if (result.modifiedCount === 0) {
        console.warn(`[Order Outbox] Event ${event.eventId} was published but worker no longer owns the lock.`);
        continue;
      }

      console.log(`[Order Outbox] Successfully processed event ${event.eventId}`);

    } catch (error) {
      console.error(`[Order Outbox] Failed to publish ${event.eventId}:`, error);

      const nextAttemptAt = calculateNextAttempt(event.attempts);

      await markFailed(event.eventId, WORKER_ID, nextAttemptAt);

      console.log(`[Order Outbox] Retry scheduled for ${event.eventId} at ${nextAttemptAt.toISOString()}`);
    }
  }
};
...
export const startOutboxWorker = (intervalMs = 3000) => {
  ...
  setInterval(async () => {
    try {
      ...
      console.error("Order Outbox worker error:", error);
    }
  }, intervalMs);
};
